...
export default function DeviceDetailView({ device, settings, telemetry, status, history, role, readonly = false }: DeviceDetailViewProps) {
    ...
    // Get last fermentation date from history
    const lastFermentation = history && history.length > 0 ? history[0] : null;
    ...
    return (
        <div className={`min-h-screen bg-[#FAFAFA] font-sans pb-12 ${readonly ? 'w-full' : ''}`}>
            <div className={`w-full mx-auto min-h-screen flex flex-col`}>
                {/* Header */}
                <header className="bg-white/80 backdrop-blur-md border-b border-gray-100 px-4 sm:px-6 h-16 md:h-20 flex items-center justify-between sticky top-0 z-50">
                            ...
                            <div className="flex items-center gap-4 sm:gap-5">
                                ...
                                <div>
                                    <div className="flex items-center gap-2 sm:gap-3 mb-1">
                                        ...
                                        {role === 'VIEWER' && (
                                            <Badge variant="secondary" className="text-[9px] font-black px-2 py-0.5 uppercase tracking-widest rounded-lg bg-amber-100 text-amber-600">Hanya Lihat</Badge>
                                        )}
                                    </div>
                                ...
                                {lastFermentation?.endedAt && (
                                    <div className="hidden md:flex items-center gap-2 px-4 py-2 bg-gray-50 rounded-2xl border border-gray-100">
                                        ...
                                        <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest">
                                            Terakhir: {new Date(lastFermentation.endedAt).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}
                                        </span>
                                    </div>
                                )}
                ...
                <Tabs defaultValue="monitoring" className="flex-1 flex flex-col pt-6 sm:pt-10">
                    <div className="px-4 sm:px-6 mb-6 sm:mb-10 max-w-5xl mx-auto w-full">
